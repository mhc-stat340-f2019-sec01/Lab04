...
train_set <- glass %>% slice(tt_inds[[1]])
test_set <- glass %>% slice(-tt_inds[[1]])
...
test_grid <- expand.grid(
  Na = seq(from = range(train_set$Na)[1], to = range(train_set$Na)[2], length = 201),
  Al = seq(from = range(train_set$Al)[1], to = range(train_set$Al)[2], length = 201)
)
head(test_grid)
...
ggplot() +
  ...
  geom_point(data = train_set, mapping = aes(x = Na, y = Al, color = type)) +
  ggtitle("KNN, k = 5")
```
...
for(i in 1:10) {
  train_dat <- train_set %>% slice(-crossval_val_fold_inds[[i]])
  val_dat <- train_set %>% slice(crossval_val_fold_inds[[i]])
  ...
  for(k in c(5, 10, 50, 100)) {
    ...
    val_preds <- predict(knn_fit, newdata = val_dat)
    
    save_ind <- which(crossval_mse$k == k & crossval_mse$fold == i)
    crossval_mse$val_error[save_ind] <- mean(val_preds != val_dat$type)
  }
}
...
test_preds <- predict(knn_fit, newdata = test_set)

mean(test_preds != test_set$type)
```

```{r}
train_set %>% count(type)
mean("WinNF" != test_set$type)
```
